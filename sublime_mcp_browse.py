...
def _fmt_commands(data, filt):
    ...
    lines = [f"MCP: Browse Commands{' — filter: ' + filt if filt else ''}", "─" * 60, ""]
    for c in cmds:
...
class McpBrowseCommandsCommand(sublime_plugin.WindowCommand):
    def run(self, filter=""):
        if filter:
            ...
            _async_browse(self.window, "MCP: Commands", filter, _get_commands, _fmt_commands)
        else:
            self.window.show_input_panel(
                "Filter by command name (blank for all):", "",
                lambda v: self._go(v), None, None,
            )
...
def _fmt_menu_items(data, filt):
    ...
    lines = [f"MCP: Browse Menu Items{' — filter: ' + filt if filt else ''}", "─" * 60, ""]
    for e in entries:
...
class McpBrowseMenuItemsCommand(sublime_plugin.WindowCommand):
    def run(self, filter=""):
        if filter:
            ...
            _async_browse(self.window, "MCP: Menu Items", filter, _get_menu_items, _fmt_menu_items)
        else:
            self.window.show_input_panel(
    ...
    def _go(self, filt):
        from User.sublime_mcp import _get_menu_items
        _async_browse(self.window, "MCP: Menu Items", filt, _get_menu_items, _fmt_menu_items)
...
def _fmt_palette(data, filt):
    entries = data.get("entries", [])
    lines = [f"MCP: Browse Command Palette{' — filter: ' + filt if filt else ''}", "─" * 60, ""]
    for e in entries:
...
class McpBrowseCommandPaletteCommand(sublime_plugin.WindowCommand):
    def run(self, filter=""):
        if filter:
            ...
            _async_browse(self.window, "MCP: Command Palette", filter, _get_command_palette, _fmt_palette)
        else:
            self.window.show_input_panel(
    ...
    def _go(self, filt):
        from User.sublime_mcp import _get_command_palette
        _async_browse(self.window, "MCP: Command Palette", filt, _get_command_palette, _fmt_palette)
...
def _fmt_syntaxes(data, filt):
    ...
    lines = [f"MCP: Browse Syntaxes{' — filter: ' + filt if filt else ''}", "─" * 60, ""]
    for s in syns:
...
def _fmt_variables(data, filt):
    ...
    lines = [f"MCP: Browse Variables{' — filter: ' + filt if filt else ''}", "─" * 60, ""]
